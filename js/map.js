...
class HiroshimaMap {
  ...
  updateForScenario(scenarioId) {
    ...
    if (scenarioId === 'custom') {
      // In custom mode, initial state is usually handled by updateForCustomGroups
      return;
    }
  ...
  updateForCustomGroups(customGroups, palette) {
    if (!this.municipalityLayers) return;

    // customGroups is { 'hiroshima': [...], 'unassigned': [...], ... }

    // Create mapping: name -> color
    const colorMap = {};
    ...
    Object.keys(customGroups).forEach((key) => {
      ...
      // Use official region color if key matches
      if (typeof MEDICAL_DATA !== 'undefined' && MEDICAL_DATA.regions[key]) {
        color = MEDICAL_DATA.regions[key].color;
      } else {
        // Fallback logic if we ever support custom new groups
        color = palette[0];
      }

      customGroups[key].forEach(name => {
        colorMap[name] = color;
      });
    });

    this.municipalityLayers.forEach(({ layer, name, regionId }) => {
      // Default style
      let color = '#cbd5e1'; // Gray for unassigned
      let weight = 1;
      let opacity = 0.2;
      ...
      if (colorMap[name]) {
        ...
        // Unassigned: Keep original region color but very faint? Or Gray?
        // Let's use gray to encourage assignment
        fillColor = '#f1f5f9';
        color = '#94a3b8';
      }
